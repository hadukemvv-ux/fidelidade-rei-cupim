...
export default function SaiposPage() {
  ...
  async function diagnose() {
    ...
    try {
      const response = await fetchAdmin(`/api/admin/saipos/diagnostico?dia=${encodeURIComponent(day)}`, { cache: 'no-store' });
  ...
  return <div className="space-y-8">
    ...
    {data?.ok && <>
      <section className="admin-notice"><strong>Conexão confirmada</strong><span>{data.vendas_encontradas} venda(s) encontrada(s) para {data.dia}. {data.observacao}</span></section>
      <section>
