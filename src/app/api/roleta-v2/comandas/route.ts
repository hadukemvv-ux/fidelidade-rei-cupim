...
import { dataExpiracaoComanda, RETENCAO_COMANDA_HORAS } from "@/lib/comandaRetention";
...
export async function POST(request: Request) {
  ...
  const expiraEm = dataExpiracaoComanda();
  const { data, error } = await supabaseAdmin.from("comandas_roleta").insert({ imagem_path: paths[0], mesa_referencia: mesa, criado_por: actor.userId, criado_por_nome: actor.nome, expira_em: expiraEm.toISOString() }).select("id, status, mesa_referencia, criado_em, expira_em").single();
  ...
  await supabaseAdmin.from("administracao_eventos").insert({
    ...
    detalhes: { mesa_referencia: mesa, imagens_privadas: 2, retencao_horas: RETENCAO_COMANDA_HORAS, expira_em: expiraEm.toISOString() },
  });
