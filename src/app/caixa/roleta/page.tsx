...
export default function CaixaRoletaPage() {
  ...
  async function criarQr() {
    ...
    try {
      ...
      const valorComanda = Number(valor.replace(/\./g, '').replace(',', '.'));
      const response = await fetch('/api/roleta-v2/sessoes', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${data.session?.access_token || ''}` }, body: JSON.stringify({ valor_comanda: valorComanda, mesa_referencia: mesa }) });
  ...
  return <main className="min-h-screen bg-stone-950 px-4 py-8 text-white"><section className="mx-auto grid max-w-xl gap-5 rounded-2xl border border-amber-500/40 bg-stone-900 p-6 shadow-2xl"><header><Link href="/caixa" className="text-sm font-bold text-amber-400 underline">← Validador de cupom</Link><p className="mt-5 text-xs font-bold uppercase tracking-[.2em] text-amber-400">Piloto controlado · Roleta V2</p><h1 className="mt-2 text-3xl font-black">Gerar QR da mesa</h1><p className="mt-2 text-sm text-stone-300">O QR expira e só pode gerar um giro. A faixa da roleta é calculada pelo sistema a partir do valor informado.</p></header>
